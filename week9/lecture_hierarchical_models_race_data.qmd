...
ggplot(running, aes(x = net)) +
  ...
  labs(title = "Net Times", x = "Net", y = "Frequency")
```

```{r}
ggplot(running, aes(x = age, y = net, group = runner, color = factor(runner))) +
  geom_line() +
  geom_point() +
  labs(title = "Net Time as Aging", x = "Age", y = "Net")
```
...
ggplot(running, aes(x = net, y = age)) +
  ...
  labs(title = "Net Time vs Age", x = "Net", y = "Age")
```
...
ggplot(running, aes(x = age, y = net)) +
  ...
  labs(title = "Net Time by Age", x = "Age", y = "Net Time") +
  theme_minimal()
```
