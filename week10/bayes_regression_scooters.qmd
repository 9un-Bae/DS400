...
```{r}
ggplot(bikes_scooters, aes(x = temp_actual)) +
  geom_histogram(binwidth = 5, fill = "blue", color = "black") +
  labs(title = "Distribution of Actual Temperature",
       x = "Temperature (°F)",
       y = "Frequency")
```

```{r}
ggplot(bikes_scooters, aes(x = scooter_rides)) +
  geom_histogram(binwidth = 50, fill = "purple", color = "black") +
  labs(title = "Distribution of Number of Scooter Rides",
       x = "Number of Scooter Rides",
       y = "Frequency")
```
...
model <- lm(rides ~ temp_actual, data = bikes_scooters)
summary(model)
```

```{r}
daily_scooter_rides <- bikes_scooters %>% 
  group_by(day_of_week) %>% 
  summarize(mean_rides = mean(scooter_rides, na.rm = TRUE))

ggplot(data = daily_scooter_rides, aes(x = day_of_week, y = mean_rides)) +
